...
model_setup_kknn = nearest_neighbor(
  mode = "regression",
  neighbors = 13,
...
for (tick in tickers) {
  ...
  ini_get_ensemble_data = df_prices$target_date[round(nrow(df_prices)/2)]
  ...
  for (ini_test in int_ensemble) {
    ...
    target_pred$.pred_kknn = predict(model_kknn, df_test_transf)$.pred
  ...
  fst::write_fst(df_full_predictions, "predictions_for_ensemble.fst", compress = 100)
  cat("\nData saved!\n")
